...
class BankIntegrationService implements IBankIntegrationService
{
    ...
    public function connectUserToBank(User $user, string $bankCode, array $credentials): BankConnection
    {
        ...
        try {
            ...
            DB::rollBack();
            Log::error('Failed to connect user to bank', [
                'user_id' => $user->uuid,
                'bank_code' => $bankCode,
                'error' => $e->getMessage(),
            ]);
    ...
    {
        return DB::transaction(function () use ($user, $bankCode) {
    ...
    public function syncBankAccounts(User $user, string $bankCode): Collection
    {
        ...
        DB::transaction(function () use ($accounts, $user, $bankCode) {
        ...
        return Cache::remember($cacheKey, 300, function () use ($user, $currency) {
            ...
            foreach ($connections as $connection) {
                ...
                try {
                    ...
                    Log::warning('Failed to get balance from bank', [
